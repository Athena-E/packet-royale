...
import { initiateCapture, canAttackEnemyBase, launchAttack } from '../utils/graphData';
...
export class UIScene extends Phaser.Scene {
  ...
  private createTopBar() {
    ...
    const throughputLabel = this.add.text(280, 20, 'THROUGHPUT:', {
      fontSize: '16px',
      fontFamily: 'monospace',
      color: '#99ccff',
    });
    ...
    this.throughputValueText = this.add.text(420, 18, '0.0 Gbps', {
      fontSize: '20px',
      fontFamily: 'monospace',
      color: '#00ff88',
      fontStyle: 'bold',
    ...
    this.nodeCountText = this.add.text(280, 38, 'NODES: 0/10', {
      fontSize: '14px',
      fontFamily: 'monospace',
      color: '#99ccff',
    });
    ...
    playerIndicator.fillCircle(250, 30, 8);
    playerIndicator.lineStyle(2, COLORS.PLAYER_1, 0.5);
    playerIndicator.strokeCircle(250, 30, 12);
    playerIndicator.setScrollFactor(0);

    const playerLabel = this.add.text(220, 20, 'YOU', {
      fontSize: '16px',
      fontFamily: 'monospace',
      color: '#00ffff',
    });
    ...
    enemyIndicator.fillCircle(width - 250, 30, 8);
    enemyIndicator.lineStyle(2, COLORS.PLAYER_2, 0.5);
    enemyIndicator.strokeCircle(width - 250, 30, 12);
    enemyIndicator.setScrollFactor(0);

    const enemyLabel = this.add.text(width - 300, 20, 'ENEMY', {
      fontSize: '16px',
      fontFamily: 'monospace',
      color: '#ff006e',
    });
    ...
    this.enemyThroughputText = this.add.text(width - 420, 18, '0.0 Gbps', {
      fontSize: '20px',
      fontFamily: 'monospace',
      color: '#ff6699',
      fontStyle: 'bold',
    ...
    this.enemyNodeCountText = this.add.text(width - 280, 38, 'NODES: 0/10', {
      fontSize: '14px',
      fontFamily: 'monospace',
      color: '#ff99bb',
    });
  ...
  private createBottomBar() {
    ...
    this.buildButton = this.createButton(50, buttonY, '[ CAPTURE NODE ]', () => {
      this.handleCaptureNode();
    });

    // Upgrade Node button (future feature)
    this.upgradeButton = this.createButton(300, buttonY, '[ UPGRADE NODE ]', () => {
    ...
    // Attack button
    this.attackButton = this.createButton(550, buttonY, '[ LAUNCH ATTACK ]', () => {
      this.handleLaunchAttack();
    });
    this.attackButton.setAlpha(0.5); // Disabled initially

    // Instructions text
    const instructions = this.add.text(850, buttonY - 10,
  ...
  private handleLaunchAttack() {
    ...
    if (!canAttackEnemyBase(this.gameState, playerId)) {
      console.log('⚠ Cannot attack yet - you must control nodes adjacent to enemy base');
      return;
    }
  ...
  private createScanlineEffect() {
    ...
    this.time.addEvent({
      ...
      callback: () => {
        ...
        this.scanline.lineBetween(0, this.scanlineY - 1, width, this.scanlineY - 1);
        this.scanline.lineBetween(0, this.scanlineY + 1, width, this.scanlineY + 1);
      },
      loop: true,
    });
  ...
  private updateUI(gameState: GameState | NetworkGameState) {
    ...
    if (player) {
      this.throughputValueText.setText(`${player.totalThroughput.toFixed(1)} Gbps`);
      this.nodeCountText.setText(`NODES: ${player.nodesOwned}/${player.maxNodes}`);
    }

    // Update enemy stats (Player 2)
    const enemy = gameState.players[1];
    if (enemy) {
      this.enemyThroughputText.setText(`${enemy.totalThroughput.toFixed(1)} Gbps`);
      this.enemyNodeCountText.setText(`NODES: ${enemy.nodesOwned}/${enemy.maxNodes}`);
    }
  ...
  private updateButtonStates() {
    ...
    const canAttack = canAttackEnemyBase(this.gameState, this.gameState.currentPlayerId);
  ...
  private showAttackConfirmation() {
    ...
    // Warning text
    const warningText = this.add.text(width / 2, height / 2 - 100,
    ...
    // Subtitle
    const subtitle = this.add.text(width / 2, height / 2 - 50,
      'TARGET ACQUIRED', {
      fontSize: '20px',
      fontFamily: 'monospace',
      color: '#00ffff',
    ...
    // Initiate button
    const initiateButton = this.createButton(width / 2 - 100, height / 2 + 20,
      '[ INITIATE ATTACK ]', () => {
      if (this.gameState) {
        const success = launchAttack(this.gameState, this.gameState.currentPlayerId);
        if (success) {
    ...
    // Cancel button
    const cancelButton = this.createButton(width / 2 - 80, height / 2 + 80,
      '[ CANCEL ]', () => {
      overlay.destroy();
      warningText.destroy();
      subtitle.destroy();
